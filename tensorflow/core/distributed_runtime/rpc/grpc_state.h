...
// is greater than 0, the request will be retried for any transient failures.
template <class Response>
class DataChannelRPCState : public GrpcClientCQTag {
 public:
  DataChannelRPCState(::grpc::GenericStub* stub, ::grpc::CompletionQueue* cq,
                      const ::grpc::string& method,
                      const protobuf::Message& request, Response* response,
                      ::grpc::ByteBuffer* payload, StatusCallback done,
                      CallOptions* call_opts, thread::ThreadPool* threadpool,
                      int32_t max_retries = 0, bool fail_fast = true,
                      const string* target = nullptr)
      : DataChannelRPCState(
            stub, cq, method, request, response, payload, std::move(done),
  ...
  DataChannelRPCState(::grpc::GenericStub* stub, ::grpc::CompletionQueue* cq,
                      const ::grpc::string& method, const Request& request,
                      Response* response, ::grpc::ByteBuffer* payload,
                      StatusCallback done, CallOptions* call_opts,
                      thread::ThreadPool* threadpool, bool fail_fast,
                      int64_t timeout_in_ms, int32_t max_retries,
                      const string* target)
      : call_opts_(call_opts),
        threadpool_(threadpool),
        done_(std::move(done)),
  ...
  void ParseAndCallDone() {
    ...
    bool parse_ok = response_buf_.Dump(&slices).ok();
    size_t slice_idx = 0;
    int64_t payload_len;
    ...
    *payload_ =
        ::grpc::ByteBuffer(payload_slices.data(), payload_slices.size());
    response_buf_ = ::grpc::ByteBuffer(slices.data() + slice_idx,
                                       slices.size() - slice_idx);

    ::grpc::ProtoBufferReader reader(&response_buf_);
    parse_ok &= response_->ParseFromZeroCopyStream(&reader);
